...
#NOTE perhaps favorite_index could be replaced by favorites_data[].active?
...
class NODEBOOSTER_OT_favorite_teleport(bpy.types.Operator):
    ...
    teleport_ng_name : bpy.props.StringProperty(
    ...
    def execute(self, context):
        ...
        match self.mode:
            ...
            case 'LOOP':
                ...
                nodetoteleport = get_favorites(ng, at_index=sett_scene.favorite_index)
            ...
            case 'TELEPORT':
                ...
                if (ng.name!=self.teleport_ng_name):
                    target_ng = bpy.data.node_groups.get(self.teleport_ng_name)
                    if target_ng:
                        space_data.pin = True
                        space_data.node_tree = target_ng
                        ng = target_ng
                    else:
                        self.report({'WARNING'}, f"Node group '{self.teleport_ng_name}' not found")
                        return {"CANCELLED"}
        ...
        for favdata in sett_scene.favorites_data:
            favdata.active = favdata.name == nodetoteleport.name and favdata.nodetree_reference == ng
...
class NODEBOOSTER_OT_favorite_remove(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        for i, fav in enumerate(sett_scene.favorites_data):
            if fav.name == self.favorite_name and fav.nodetree_reference == ng:
...
class NODEBOOSTER_PT_favorites_popover(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        for ng in sorted_nodetrees:
            ...
            headercol = layout.column(align=True)
            headercol.separator(type='LINE')
            header_row = headercol.row(align=True)
            header_row.label(text=ng.name, icon=headericon)
            headercol.separator(type='LINE')
